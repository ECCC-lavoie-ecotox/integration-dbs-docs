...
```{r}
con <- DBI::dbConnect(RSQLite::SQLite(), "C:/Users/VissaultS/Documents/git/integration-dbs-docs/contaminants-rlavoie-eccc.sqlite")
...
## Add undocumented lab_sample_id in lab_sample tab
undocumented_lab_sample_id <- measurements |> 
  dplyr::filter(!id_lab_sample %in% lab_samples$id_lab_sample) |>
  dplyr::pull(id_lab_sample) |> 
  unique()

undocumented_lab_sample <- data.frame(
  id_lab_sample = undocumented_lab_sample_id, 
  id_field_sample = NA,
  id_report = NA,
  note = NA
)

DBI::dbWriteTable(con, "lab_sample", undocumented_lab_sample, append = TRUE)
```
